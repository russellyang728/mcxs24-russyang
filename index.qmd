...
\end{align}
$$
$\kappa_1$ is the overall shrinkage level for autoregressive slopes with common value $0.02^{2}$.
...
The following graphs are impulse response of basic model after real data are incorporated. The response of unemployment rate is relatively flat, indicating that the shock might have a limited impact on unemployment. So do real GDP, inflation, and cash rate. Positive effect shows by the increasing trend of tax revenue while slightly decreasing trend suggests that the shock leads to a reduction in government expenditure over time.
...
restriction = diag(c(-1,1,-1,-1,0,0))
i.vec = c()
Q.store = array(NA,c(N,N,S))
B0.store = array(NA,c(N,N,S))
Bplus.store = array(NA,c(N,K,S))
...
for (s in 1:S){
  B = solve(B0.store[,,s])
  B.posterior[,,s]= B 
  A.posterior[,,s]= t(B %*% Bplus.store[,,s]) 
}

...
draws = Posterior.draws(S = 1000, Posterior_param = Posterior_param)

```

Then we employed sign restriction into the posterior draws.
...
A.store <- array(NA,c(N+1,N,S))
Sigma.store <- array(NA,c(N,N,S))

Restriction_output = WithSignRestriction(restriction = diag(c(1,1)),
                                         N=N,p=p,S=S,Posterior.draws=draws )

B0.store = Restriction_output$B0.store
Bplus.store = Restriction_output$Bplus.store

for (s in 1:S){
  # convert B0 into Sigma 
  Sigma.store[,,s] <- solve(B0.store[,,s]) %*% t(solve(B0.store[,,s]))
  A.store[,,s] <- t(solve(B0.store[,,s] )%*% Bplus.store[,,s])
}
...
Sigma_mean <- apply(Sigma.store, 1:2, mean)
...
B0_mean <- apply(B0.store, 1:2, mean)
B0_df <- as.data.frame(B0_mean)
colnames(B0_df) <- c("Y1-Lag", "Y2-Lag")
rownames(B0_df) <- c("Simulation_Y1", "Simulation_Y2")
knitr::kable(Sigma_df, caption = "Posterior Mean of B0")
...
Bplus_mean <- apply(Bplus.store, 1:2, mean)
Bplus_df <- as.data.frame(Bplus_mean)
...
GetPrior.parameters.ext <- function (N,p,Sigma.hat){
  kappa.1     <- 1
  kappa.2     <- 100
  ...
  A.prior[2:(N+1),] <- diag(N)
  V.prior     <- diag(c(kappa.2,kappa.1*((1:p)^(-2))%x%rep(1,N)))
  Sigma.prior     <- diag(diag(Sigma.hat))
  nu.prior    <- N+1
...
# find the prior for the sample
Prior.ext = GetPrior.parameters.ext(N=N,p=p,Sigma.hat=Sigma.hat)
...
Posterior.param.ext = Posterior.parameters.ext(X,Y,Prior.ext,S)
...
Posterior.draws.ext = function(S, Posterior.param.ext){
  
  A.bar  <- Posterior.param.ext$A.bar
  V.bar  <- Posterior.param.ext$V.bar
  Sigma.bar  <- Posterior.param.ext$Sigma.bar
  nu.bar <- Posterior.param.ext$nu.bar
  
  B0.tilde <- array(NA,c(N,N,S))
  Bplus.tilde <- array(NA,c(N,K,S))
  L        <- t(chol(V.bar)) 
  
  Sigma.posterior   <- rWishart(S, df=nu.bar, Sigma=solve(Sigma.bar))  
  Sigma.posterior   <- apply(Sigma.posterior,3,solve)            
  Sigma.posterior   <- array(Sigma.posterior,c(N,N,S))   
  A.posterior       <- array(rnorm(prod(c(dim(A.bar),S))),c(dim(A.bar),S))
...
draws.ext = Posterior.draws.ext(S , Posterior.param.ext = Posterior.param.ext)
...
WithSignRestriction.ext <- function(restriction,N,p,S,Posterior.draws.ext){
  
  A.posterior = draws.ext$A.posterior
  Sigma.posterior = draws.ext$Sigma.posterior
  B0.tilde = draws.ext$B0.tilde
  Bplus.tilde = draws.ext$Bplus.tilde
...
Restriction.output.ext = WithSignRestriction.ext(restriction = diag(c(1,1)),
                                                 N=N,p=p,S=S,Posterior.draws.ext=draws.ext)

B0.store = Restriction.output.ext$B0.store
Bplus.store = Restriction.output.ext$Bplus.store
...
a.posterior = Posterior.param.ext$a.posterior
a_mean <- data.frame(a = mean(a.posterior))
knitr::kable(a_mean,align = 'c',index = FALSE, caption = "Posterior Mean of a")
...
A_mean_ext <- apply(A.store, 1:2, mean)
A_df_ext <- as.data.frame(A_mean_ext)
colnames(A_df_ext) <- c("Simulation_Y1", "Simulation_Y2")
rownames(A_df_ext) <- c("Constant", "Y1-Lag", "Y2-Lag")
knitr::kable(A_df_ext, caption = "Posterior Mean of A_ext")
...
Sigma_mean_ext <- apply(Sigma.store, 1:2, mean)
Sigma_df_ext <- as.data.frame(Sigma_mean_ext)
colnames(Sigma_df_ext) <- c("Simulation_Y1ext", "Simulation_Y2ext")
rownames(Sigma_df_ext) <- c("Y1-Lag", "Y2-Lag")
knitr::kable(Sigma_df_ext, caption = "Posterior Mean of Sigma_ext")
...
B0_mean_ext <- apply(B0.store, 1:2, mean)
B0_df_ext <- as.data.frame(B0_mean_ext)
colnames(B0_df_ext) <- c("Y1-Lag", "Y2-Lag")
rownames(B0_df_ext) <- c("Simulation_Y1ext", "Simulation_Y2ext")
knitr::kable(B0_df_ext, caption = "Posterior Mean of B0_ext")
...
Bplus_mean_ext <- apply(Bplus.store, 1:2, mean)
Bplus_df_ext <- as.data.frame(Bplus_mean_ext)
colnames(Bplus_df_ext) <- c("Y1-Lag", "Y2-Lag")
rownames(Bplus_df_ext) <- c("Simulation_Y1ext", "Simulation_Y2ext")
knitr::kable(Bplus_df_ext, caption = "Posterior Mean of Bplus_ext")
...
Then the likelihood function looks as follow: 
$$
\begin{gather}
...
###############
Posterior_param = Posterior_parameters(X=X,Y=Y,A=Prior$A.prior,V=Prior$V.prior, 
                                       S=Prior$S.prior,nu=Prior$nu.prior)

# posterior draws 
###############
draws = Posterior.draws(S, Posterior_param = Posterior_param)
...
Restriction_output = WithSignRestriction(restriction = diag(c(0,1,-1,-1,0,0)),
                                         N=N,p=p,S=S,Posterior.draws=draws )
...
mean_A <- round(apply(draws$A.posterior,1:2,mean),4)
colnames(mean_A) <- colnames(data_df)
...
#the mean of posterior draws of A matrices
mean_Sigma <- round(apply(draws$Sigma.posterior,1:2,mean),4)
colnames(mean_Sigma) <- colnames(data_df)
...
Prior.sv <- GetPrior.parameters(N,p,Sigma.hat)
Posterior.sv <- DrawPosterior.sv(Y,X,Prior.sv,500,4000)

sigma2.mean.vec <- apply(Posterior.sv$H.sv,1,mean) 
...
The plots the S2 draws of Gibbs Sampler of the Stochastic Volatility model, and it shown to be stationary for A and $\Sigma$ when $S_1=500, S_2=4000$.
$A_{21}$ and $A_{32}$ plots: These parameters demonstrate substantial stability in sampling, particularly for $A_{21}$, which fluctuates between $0.9$ and $1.1$. In contrast, $A_{32}$ varies more widely between $-10$ and $10$, indicating greater variability.
$Σ_{21}$ and $Σ_{22}$ plots: The $Σ_{21}$ parameter shows relatively low variability with small values, suggesting stable estimates. Conversely, $Σ_{22}$ exhibits larger fluctuations, particularly at specific points, suggesting significant changes in volatility estimates at these points.
...
plot(Posterior.sv$A.posterior[2,1,],type='l',col="#660099",ylab="",xlab="",main=expression(A[21]), lwd = 0.1)
plot(Posterior.sv$A.posterior[3,2,],type='l',col="#CC66CC",ylab="",xlab="",main=expression(A[32]), lwd = 0.1)
plot(Posterior.sv$Sigma.posterior[1,1,],type='l',col="#FFA500",ylab="",xlab="",main=expression(Sigma[21]), lwd = 0.1)
plot(Posterior.sv$Sigma.posterior[2,2,],type='l',col="#FFD700",ylab="",xlab="",main=expression(Sigma[22]), lwd = 0.1)
...
```{r}
#| label: fig-IRF
#| fig-cap: "Impulse Response of Basic Model"
...
#the mean of posterior draws of A matrices
mean_A.ext <- round(apply(draws.ext$A.posterior,1:2,mean),4)
colnames(mean_A.ext) <- colnames(data_df)
knitr::kable(mean_A.ext, caption = "Posterior Mean of A_ext",index=TRUE)
...
#the mean of posterior draws of A matrices
mean_Sigma.ext <- round(apply(draws.ext$Sigma.posterior,1:2,mean),4)
colnames(mean_Sigma.ext) <- colnames(data_df)
knitr::kable(mean_Sigma.ext, caption = "Posterior Mean of Sigma_ext",index=TRUE)
...
```{r}
#| label: fig-pos-svts-ext
#| fig-cap: "Posterior Draw Mean of Stochastic Volatility of Extension Model"

Prior.sv.ext <- GetPrior.parameters.ext(N,p,Sigma.hat)
Posterior.sv.ext <- DrawPosterior.sv.ext(Y,X,Prior.sv.ext,500,4000)

sigma2.mean.vec <- apply(Posterior.sv.ext$H.sv,1,mean) 
plot.ts(sigma2.mean.vec,main=expression(paste("Posterior Mean of ",sigma^2)),
        ylab="",type='l',lwd=2,col="#9E9AC8",xlab="T")
abline(h=mean(sigma2.mean.vec),lty=2)
...
```{r}
#| label: fig-Gibbs-Sampler-SV-ext
#| fig-cap: "Gibbs Sampler draws of A and Sigma-SV model of Extension Model"

par(mfrow=c(2,2),mar=c(2,2,2,2))

plot(Posterior.sv.ext$A.posterior[2,1,],type='l',col="#8B0000",ylab="",xlab="",main=expression(A[21]), lwd = 0.1)
plot(Posterior.sv.ext$A.posterior[3,2,],type='l',col="#FFB6C1",ylab="",xlab="",main=expression(A[32]), lwd = 0.1)
plot(Posterior.sv.ext$Sigma.posterior[1,1,],type='l',col="#1E90FF",ylab="",xlab="",main=expression(Sigma[21]), lwd = 0.1)
plot(Posterior.sv.ext$Sigma.posterior[2,2,],type='l',col="#ADD8E6",ylab="",xlab="",main=expression(Sigma[22]), lwd = 0.1)
...
```{r}
#| label: fig-IRF-ext
#| fig-cap: "Impulse Response of Extension Model"
par(mfrow=c(4,2), mar=c(2,2,2,2),cex.axis=1, cex.lab=1)
for (n in 1:N){
  ylims     = range(IRFs.k1[n,hh],IRFs.k1.hdi[,n,1:(h+1)],0)
  plot(hh,IRFs.k1[n,hh], type="l", ylim=ylims, axes=FALSE, xlab="", main=names[n])

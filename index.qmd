...
```{=tex}
\begin{align*}
\mathbf{A}| \Sigma &\sim \mathcal{MN}_{K \times N}(\underline{\mathbf{A}}, \underline{\mathbf{V}}, \Sigma) \\
...
&\propto |\Sigma|^{-T/2} \exp \left\{ -\frac{1}{2} \text{tr}\left[ R + (A - \hat{A})'X'X(A - \hat{A})\Sigma^{-1}\right] \right\} \\
&\quad \times |\Sigma|^{-T/2} \exp \left\{ -\frac{1}{2} \text{tr}\left[(A - \underline{A})'\underline{V}^{-1}(A - \underline{A})\Sigma^{-1}\right] \right\} \\
&\quad \times |\Sigma|^{-(\underline{\nu}+N+1)/2} \exp \left\{ -\frac{1}{2} \text{tr}(\underline{S}\Sigma^{-1}) \right\} \\
...
```{=tex}
\begin{align*}
A|\Sigma,Y &\sim \mathcal{MN}_{K\times N}(\overline{A}, \overline{V},\Sigma) \\
...
To empirically test the hypotheses outlined in section 3.3, we employ a SVAR model complemented by sign restrictions that reflect our theoretical expectations about the economy. Sign restrictions are a non-intrusive way to impose economic theory in SVAR analysis, allowing the data to identify the model within the confines of the signs imposed on the impulse responses. Based on assumptions above, we have following sign restriction\[Table 2: Sign Restriction\]: 
